...
public class EventSubclassTransformer {
    public Optional<ClassNode> transform(final ClassNode classNode, final Type classType) {
        try {
            ...
            FabricatedForgeEventBus.LOGGER.error(FabricatedForgeEventBus.EVENTBUS, "An error occurred building event handler", e);
        }
    ...
    private boolean buildEvents(ClassNode classNode) throws Exception {
        ...
        try {
            ...
            FabricatedForgeEventBus.LOGGER.error(FabricatedForgeEventBus.EVENTBUS, "Could not find parent {} for class {} in classloader {} on thread {}", classNode.superName, classNode.name, Thread.currentThread().getContextClassLoader(), Thread.currentThread());
            throw e;
        }
        ...
        FabricatedForgeEventBus.LOGGER.debug(FabricatedForgeEventBus.EVENTBUS, "Event transform begin: {}", classNode.name);
        ...
        if (hasGetListenerList) {
            FabricatedForgeEventBus.LOGGER.debug(FabricatedForgeEventBus.EVENTBUS, "Transforming event complete - already done: {}", classNode.name);
            return true;
        }
    ...
    private boolean addListenerList(ClassNode classNode, boolean useSuper) {
        ...
        FabricatedForgeEventBus.LOGGER.debug(FabricatedForgeEventBus.EVENTBUS, "Event transform complete: {}", classNode.name);
        return true;
    }
}
