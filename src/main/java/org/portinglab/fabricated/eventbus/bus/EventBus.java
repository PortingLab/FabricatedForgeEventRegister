...
import org.portinglab.fabricated.eventbus.bus.api.*;
...
public class EventBus implements EventExceptionHandlerImpl, EventBusImpl {
    ...
    private <T extends Event> Class<T> getEventClass(Consumer<T> consumer) {
        ...
        if ((Class<?>)eventClass == TypeResolver.Unknown.class) {
            FabricatedForgeEventBus.LOGGER.error(FabricatedForgeEventBus.EVENTBUS, "Failed to resolve handler for \"{}\"", consumer.toString());
    ...
    private <T extends Event> void addListener(final EventPriority priority, final Predicate<? super T> filter, final Consumer<T> consumer) {
        ...
        if (Objects.equals(eventClass, Event.class))
            FabricatedForgeEventBus.LOGGER.warn(FabricatedForgeEventBus.EVENTBUS,"Attempting to add a Lambda listener with computed generic type of Event. " +
    ...
    private void register(Class<?> eventType, Object target, Method method)
    {
        try {
            ...
            FabricatedForgeEventBus.LOGGER.error(FabricatedForgeEventBus.EVENTBUS,"Error registering event handler: {} {}", eventType, method, e);
        }
    }
